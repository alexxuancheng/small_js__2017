...
function getRoute(){
	...
	var url_add="http://data.foli.fi/gtfs/v0/20171130-162538/routes";
	...
	client.onreadystatechange = function() {
		if(client.readyState == 4 && client.status==200) {
			...
			for(i=0;i<l;i++){				
				var y=jsonObj[i]['route_id'];
				var z=jsonObj[i]['route_short_name'];
...
function showRoute(){
	...
	var client = new XMLHttpRequest();
	var url_add="http://data.foli.fi/gtfs/v0/20171130-162538/trips/route/"+route_id;
	client.open("GET", url_add, true);
	client.onreadystatechange = function() {
		if(client.readyState == 4 && client.status==200) {
			var jsonObj = JSON.parse(client.responseText);
			var shape_id=jsonObj[0]['shape_id'];

			// Acquire coordinate listing by shapes/shape_id table
			var url_add2="http://data.foli.fi/gtfs/v0/20171130-162538/shapes/"+shape_id;
			...
			client.onreadystatechange = function() {
				if(client.readyState == 4 && client.status==200) {
					...
					function myMap() {
						var myCenter = new google.maps.LatLng(jsonObj2[d]['lat'],jsonObj2[d]['lon']);//set the middle of the path of the center of the map
						var mapCanvas = document.getElementById("googleMap");
						var mapOptions = {center: myCenter, zoom: 12};
						var map = new google.maps.Map(mapCanvas, mapOptions);
...
function showBuses(){
	...
	var client = new XMLHttpRequest();
	var url_add3="http://data.foli.fi/siri/vm/pretty";
			client.open("GET", url_add3, true);
			client.onreadystatechange = function() {
				if(client.readyState == 4 && client.status==200) {
					var jsonObj3 = JSON.parse(client.responseText);	
					var myObj=jsonObj3['result']['vehicles'];
					...
					function myMap() {
						...
						var map = new google.maps.Map(mapCanvas, mapOptions);
						for(i=0;i<l;i++){
							var vehicle_number=jsonObj3['result']['vehicles'][vehicle_number_list[i]];
